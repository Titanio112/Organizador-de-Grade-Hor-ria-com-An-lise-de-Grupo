...
- [x] AUDITORIA FINAL DE BACKEND (2026-09-16, modo QA severo): test_stress.js 17/17 — integridade FKs, cascata without orfaos, choque cross-grade, absences negativo, status invalido, horario invertido, DELETE fora do escopo isolado. Nenhuma falha real no banco; 2 ajustes de teste (trigger barra antes da FK = defesa em camadas; PostgREST exige filtro em DELETE). Sign-off: docs/BACKEND_AUDIT.md — backend aprovado, UI desbloqueada. Classe total: 61/61 em todas as baterias. LINEBREAK- [x] Normalizacao v4 (2026-09-16): rooms (catálogo, UNIQUE(name,campus_id), idx_rooms_name) + schedule_rooms (N:N sched<->sala); coluna room removida de class_schedules. splitSalas() com guarda anti-N/A. Seed em lote (unnest) rapido e idempotente com remap de IDs. Final: 12 salas, 65 classes, 89 blocos, 86 vinculos sala. Bug achado e corrigido: profiles sem policy INSERT (usuarios legados nao conseguiam recriar perfil) -> policy adicionada no schema e aplicada. Regressao: 44/44 (api 6/6, trigger 10/10, mock 16/16, normalizacao 12/12).
...
- [ ] BLOQUEADORES LANC. (Parte 1 briefing): (a) ROTACIONAR senha do banco — ainda ativa e vazada em 3 commits do historico local (remoto nunca recebeu push — janela perfeita pra reescrever antes); (b) SMTP custom no Supabase free (rate limit de email) antes de abrir cadastro; (c) criar repo remoto grade-horaria (ls-remote = not found).
- [ ] Parte 2 briefing: SCHEMA V5 PLANEJADO E NAO APLICADO (docs/SCHEMA_V5_PLAN.md) — aguardando revisao do usuario. Resumo: friendships+groups+friend_code; visibility enum(private/friends/public); admin 3 niveis com admin_grants+granted_by+grant_admin; notifications+triggers; reports; cores por perfil. Aberto: 8 perguntas no doc.
