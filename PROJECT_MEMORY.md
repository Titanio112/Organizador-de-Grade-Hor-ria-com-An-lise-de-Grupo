...
- [x] Parte 1 briefing bloqueadores: historico git limpo via git filter-repo (senha removida de TODOS os commits; remote ainda nao criado — criar apos checar); RESTANTE MANUAL: rotacionar senha no dashboard Supabase + configurar SMTP custom + criar repo remoto.
- [x] SCHEMA V5 APLICADO (2026-09-18): friendships (anti-par-inverso via UNIQUE funcional LEAST/GREATEST), groups+group_members, profiles.friend_code (retry em colisao no handle_new_user) + color_preset, grades.visibility(private/friends/public) substitui is_public, admin_grants (super/institution/course com granted_by) + grant_admin() com cascata de privilegio, notifications com dedupe_key (anti-spam turma+dia) + triggers de mudanca (classes/class_schedules/schedule_rooms), reports roteados por escopo. Rollback: migrations/v5_down.sql. check_v5.js confirma: 20 tabelas, funcoes ok.
- [x] Bateria v5: test_v5.js 25/25. Regressao total v3->v5: mock 16/16, trigger 10/10, stress 17/17, normalization 12/12, api 6/6 = 86/86.
- [x] Bug importante no processo: politcas de groups/group_members com auto-join causavam "infinite recursion" (42P17) — corrigido com funcao is_group_member/is_group_creator SECURITY DEFINER.
- [x] Dado real corrigido no banco: metodologia virou Seg 13:05-15:00 (fonte: banco). Testes atualizados pra refletir o dado real.
- [x] AUDITORIA FINAL v4: test_stress.js 17/17 - integridade FKs, cascata sem orfaos, choque cross-grade, absences negativo, status invalido, horario invertido, DELETE fora do escopo isolado. Sign-off: docs/BACKEND_AUDIT.md.
- [x] Normalizacao v4 (2026-09-16): rooms (UNIQUE(name,campus_id), idx) + schedule_rooms N:N; coluna room removida de class_schedules; bug profiles sem policy INSERT corrigido. Regressao 44/44.
...
- [x] Pre-lancamento: SENHA ROTACIONADA (antiga morreu, nova em .env funcionando - provado com teste de conexao 3-versoes). Historico limpo (filter-repo). PUSH FEITO: branch master no remoto Titanio112/Organizador-de-Grade-Hor-ria-com-An-lise-de-Grupo (local master == origin/master). ATENCAO: repo remoto tem main ANTIGA (versao estatica, historico nao relacionado) - decidir depois se vira default. SMTP: adiado por decisao do usuario (confirmacao de email continua ligada).
- [x] Parte 1 verificacao independente: supabase/verify_part1.js 6/6 — schema ao vivo confere; multi-instituicao PRECISO (2a instituicao UFQA criada e destruida ok); docs/BRIEFING_PART1_REPORT.md com evidencias.
